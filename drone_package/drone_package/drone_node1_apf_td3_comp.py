...
class DroneNode1(Node):
    ...
    def subscribe_position(self, msg):
        ...
        if drone_id == self.drone_num:
            ...
            self.other_drones_positions[drone_id] = np.array([x, y, z])
    ...
    def path_planning(self):
        ...
        if distance_to_goal <= self.landing_threshold:  # landing condition
            ...
            state = np.pad(state, (0, expected_obs_shape - len(state)), mode='constant')
            action, _states = self.model.predict(state, deterministic=True)

            a = action[0]
            b = [action[1], action[2], action[3]]
...
def main(args=None):
    ...
    args = sys.argv[1:]
    if len(args) != 3:
        print("Usage: ros2 run drone_package drone_node1 <goal_x> <goal_y> <goal_z>")
        return
    ...
    drone1 = DroneNode1(goal_x, goal_y, goal_z)
    try:
        rclpy.spin(drone1)
    finally:
